...
namespace Server
{
    public  partial class Form1 : Form
    {
        ...
        public List<User> users = new List<User>(); //aktywni użytkownicy
        
        List<Socket> soki = new List<Socket>(); //klientowe
        
        List<Thread> trid = new List<Thread>(); //Thready klienta

        public List<String> loginy = new List<String>(); //do sczytania loginów - wszytkich z pliku
                ...
                using (StreamReader sr = new StreamReader("logi" + ".txt"))
                {
                    ...
                    while (wiersz != null)// po dokumencie wpierw szukamy loginu
                    {
                        ...
                        if (!(wiersz == ""))//gdy jest pusty wiersz zwraca false 
                        {
                            ...
                            loginy.Add(cut[0]); // doda wszystkie loginy do listy
                            wiersz = sr.ReadLine();
                        }
                    }
                }
        ...
        public void StartServer(Object form)
        {
            ...
            wczytanie_login(); // loginy z liku w liscie
            //proba_();
            ...
            while (true)
            {
                ...
                if (!(data == ""))//gdy nie jest pusty
                {
                    ...
                    string[] cut = data.Split('/'); // cut[0] = log or reg, [1] = login, [2]=haslo
                                                    //cut[1].Replace(',', '.');
                    klientowy = new Thread(() => baza_danych(cut[0], cut[1], cut[2], klient,brd));  //nowy Thread dla kazdego klienta
                    klientowy.Start();
        ...
        public void nasluchiwanie_info_powtorne(Socket s,User client, RichTextBox rch)//i wysylanie do pozostalych
        {
            ...
            while (true)
            {
                ...
                try
                {
                   ...
                   //rch.Text += Environment.NewLine + data;
                    ...
                    if (!(data == "")) //gdy nie jest pusty
                    {
                        ...
                        //double kurs = Double.parseDouble(cut[1]);
                         baza_danych(cut[0], cut[1], cut[2], client,rch); 
        ...
        public void  baza_danych(String opt,string log,string haslo,User client,RichTextBox rch)
        {
            /*
            int counter = 0;
            ...
            if (opt == "log")
                {
                    if (yes_or_no(log))
                    {
                        check_haslo(log,haslo,client,rch); 
                    }
                    else // 
                    {
                        byte[] msg = Encoding.ASCII.GetBytes("Zarejestruj sie/");
                        acceptedSocket.Send(msg); // nie istnieje
                        nasluchiwanie_info_powtorne(client.s,client,rch);//powtorz rejestracje
                    }
                }
           else //rejestracja
                {
                    if (yes_or_no(log)) // juz taki login jest
                    {
                        ...
                        nasluchiwanie_info_powtorne(client.s, client,rch);
                    }
                    else // nie ma w bazie loginu
                    {
                        ...
                        rch.Text += Environment.NewLine + "Aktywny-Zarejestrowany user: " + klient.name;
                        acceptedSocket.Send(msg);
                        //??Q??80?????? = zalogowany     
                    ...
                    //dodaj_uzytkownika(klient);
                     users.Add(klient);           // aktywny uzytkownik
        ...
        public void check_haslo(String login,String pass,User client, RichTextBox rch)
        {
            bool match = false;
           try
            {
                        ...
                        if (!(wiersz == ""))//gdy nie jest pusty
                        {
                            ...
                            if (cut[0] == login)
                            {
                                if (cut[1] == pass)
                                {
                                    ...
                                    if (w_skrzynce(klient.name)) //czy cos czeka w skrzynce
                                    {
                                        byte[] zalegle = Encoding.ASCII.GetBytes("Wiadomosci przeslane podczas twojej absencji: ");
                                        acceptedSocket.Send(zalegle);
                                        klient.get_post(acceptedSocket);
                                        string path = @"skrzynka\" + klient.name + ".txt";  //czyszczenie pliku po wejsciu
                                        File.WriteAllText(path, String.Empty);
                                        messages(klient);
                                    }
                                    else
                                    {
                                        byte[] zalegle = Encoding.ASCII.GetBytes("Brak Wiadomosci podczas twojej absencji: ");
                                        acceptedSocket.Send(zalegle);
                                        messages(klient);
                                    }
            ...
            catch (Exception x)
            {
                ...
                richTextBox1.Text += x.Message;          //Sprawdz watek
        ...
        /*
        public void proba_() //zapis do folderu
        {

            String zapis = "Proba"; // 
                                    //plik nazwany loginem adresaty
            const string V = "Jamie.txt";
            string path = @"C:\Users\User\Politechnika_4sem\Informatyka\19.04 - Rozbudowany Serwer z pocztą\Server\Server\bin\Debug\skrzynka\"+ V;
        ...
        */
        ...
        public async void  messages(User client)
        {

            //async zeby pozwolic czekać na API
            if (client.logged_in)
            {
                ...
                while (true)
                {
                    ...
                    if (!(data == ""))//gdy nie jest pusty
                    {
                        ...
                        //censor
                        string censored =  await censoreAsync(cut[1]);
                        string []seperator = new string[] { "\"" };
                        string []censored_result =  censored.Split(seperator, StringSplitOptions.None)
;                       cut[1] = censored_result[3];
                        ...
                        else
                        {
                            ...
                            if (!was)//nie ma takiego loginu aktywnego
                            {


                                foreach (String element in loginy)
                                {
                                   if (element == cut[0])//na liscie jest taki nieaktywny login 
                                    {
                                        skrzynka(element, klient.name + ": " + cut[1]);
                                    }
                                }
        ...
        bool w_skrzynce(string log)
        {
            try
            {
                using (StreamReader sr = new StreamReader(@"skrzynka\" + log + ".txt"))
                {
                    string wiersz = sr.ReadLine();
                    if (wiersz != null)
                    {
            ...
            {
                return false;  //nic nie czeka
            }
        }


         public void skrzynka(string log,string text)
        {
             String zapis = text; // 
             //plik nazwany loginem adresaty
            string path =  log + ".txt";
            using (StreamWriter label = new StreamWriter(@"skrzynka\" + path,true))
            {
                label.WriteLine(zapis);
            }
        }
        ...
        public  Task<String> censoreAsync(string word)
        {
            ...
            var request = new HttpRequestMessage
            {
                ...
                Headers =
      {
        { "x-rapidapi-key", "5a270d99bdmshb4eb41b66319afap11fc12jsnb6a4f2761f46" },
